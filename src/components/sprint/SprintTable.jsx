...
import { ChevronDown, MessageCircle, ChevronUp, Check, Download, Settings, Plus, Edit, Trash2, Paperclip, User, AlertTriangle, Tag, Search  } from 'lucide-react';
...
export const SprintTable = ({ proyecto, integrantes, onRefreshSprints }) => {
  const { 
    sprintsList, 
    isLoading, 
    error, 
...
const stateOptions = ["Late", "In Progress", "In Review", "Completed"];
                ...
                {expandedSprint === sprint.uid && (
                  ...
                  >
                    <div className="p-5 border-t border-[#036873]/30 text-white">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {(() => {
                        const tareasFiltradas = (isScrumMaster && showAllTasks
                          ? sprint.task
                          : sprint.task?.filter(task => 
                              isAssignedToCurrentUser(task.assignedTo) ||
                              (isScrumMaster && !showAllTasks && task.assignedTo?._id === myUser?._id)
                            )
                        )?.filter(task => {
                        ...
                        return tareasFiltradas.map(task => (
                          ...
                          <div className="grid grid-cols-2 gap-2 mb-3">
                            ...
                            {(isScrumMaster || isAssignedToCurrentUser(task.assignedTo)) && task.assignedTo && (
                              <div>
                                <h4 className="text-xs text-gray-400">Asignado a</h4>
                                <p className="text-xs">
                                  {getAssignedUserName(task.assignedTo)}
                                  {isAssignedToCurrentUser(task.assignedTo) ? " (Tú)" : ""}
                                </p>
                                {isScrumMaster && (
                                  <p className="text-xs">{getAssignedUserEmail(task.assignedTo)}</p>
                                )}
                              </div>
                            )}
                          </div>
